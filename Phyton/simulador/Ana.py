...
def rudge_ramos():
    ...
    while True:
        ...
        cursor.execute(f"INSERT INTO Registro (valor, dataHora, fkComponente, fkTotem) VALUES ({cpu_m1}, NOW(), 1, 2), ({ram_m1}, NOW(), 2, 2),({disco_m1}, NOW(), 3, 2);")
            
        conexao.commit()

        if (ram_m1 >= 85) and (ram_m1 < 95):
            if (isExibiuAlerta == False):
                cursor.execute(f"INSERT INTO Alerta (tipo, descricao, fkRegistro) VALUES (1,{ram_m1},(SELECT idRegistro FROM Registro WHERE fkTotem = 2 AND fkComponente = 2 ORDER BY dataHora DESC LIMIT 1));")
                conexao.commit()
                isExibiuAlerta = True 
                isExibiuCritico = False
        elif (ram_m1 >= 95):
            if (isExibiuCritico == False):
                cursor.execute(f"INSERT INTO Alerta (tipo, descricao, fkRegistro) VALUES (2,{ram_m1},(SELECT idRegistro FROM Registro WHERE fkTotem = 2 AND fkComponente = 2 ORDER BY dataHora DESC LIMIT 1));")
                conexao.commit()
                isExibiuAlerta = False 
        ...
        if (cpu_m1 >= 85) and (cpu_m1 < 95):
            if (isExibiuAlerta == False):
                cursor.execute(f"INSERT INTO Alerta (tipo, descricao, fkRegistro) VALUES (1,{cpu_m1},(SELECT idRegistro FROM Registro WHERE fkTotem = 2 AND fkComponente = 1 ORDER BY dataHora DESC LIMIT 1));")
                conexao.commit()
                isExibiuAlerta = True 
                isExibiuCritico = False
        elif (cpu_m1 >= 95):
            if (isExibiuCritico == False):
                cursor.execute(f"INSERT INTO Alerta (tipo, descricao, fkRegistro) VALUES (2,{cpu_m1},(SELECT idRegistro FROM Registro WHERE fkTotem = 2 AND fkComponente = 1 ORDER BY dataHora DESC LIMIT 1));")
                conexao.commit()
                isExibiuAlerta = False 
                isExibiuCritico = True 
        else: 
