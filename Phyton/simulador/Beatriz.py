...
def rudge_ramos():
    ...
    while True:
        ...
        cursor.execute(f"INSERT INTO Registro (valor, dataHora, fkComponente, fkTotem) VALUES ({cpu_m1}, NOW(), 1, 3), ({ram_m1}, NOW(), 2, 3),({disco_m1}, NOW(), 3, 3);")
            
        conexao.commit()

        if (ram_m1 >= 85) and (ram_m1 < 95):
            if (isExibiuAlertaRam == False):
                cursor.execute(f"INSERT INTO Alerta (tipo, descricao, fkRegistro) VALUES (1,{ram_m1},(SELECT idRegistro FROM Registro WHERE fkTotem = 3 AND fkComponente = 2 ORDER BY dataHora DESC LIMIT 1));")
                conexao.commit()
                isExibiuAlertaRam = True 
                isExibiuCriticoRam = False
        elif (ram_m1 >= 95):
            if (isExibiuCriticoRam == False):
                cursor.execute(f"INSERT INTO Alerta (tipo, descricao, fkRegistro) VALUES (2,{ram_m1},(SELECT idRegistro FROM Registro WHERE fkTotem = 3 AND fkComponente = 2 ORDER BY dataHora DESC LIMIT 1));")
                conexao.commit()
                isExibiuAlertaRam = False 
        ...
        if (cpu_m1 >= 85) and (cpu_m1 < 95):
            if (isExibiuAlertaCpu == False):
                cursor.execute(f"INSERT INTO Alerta (tipo, descricao, fkRegistro) VALUES (1,{cpu_m1},(SELECT idRegistro FROM Registro WHERE fkTotem = 3 AND fkComponente = 1 ORDER BY dataHora DESC LIMIT 1));")
                conexao.commit()
                isExibiuAlertaCpu = True 
                isExibiuCriticoCpu = False
        elif (cpu_m1 >= 95):
            if (isExibiuCriticoCpu == False):
                cursor.execute(f"INSERT INTO Alerta (tipo, descricao, fkRegistro) VALUES (2,{cpu_m1},(SELECT idRegistro FROM Registro WHERE fkTotem = 3 AND fkComponente = 1 ORDER BY dataHora DESC LIMIT 1));")
                conexao.commit()
                isExibiuAlertaCpu = False 
